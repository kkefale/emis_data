...
**v0.2.0** · South African Department of Basic Education · 2025 Data Year
...
| Learner / Educator Ratio | National LER across the filtered selection — shows `▲/▼ vs national` delta when filters are active |
| % No-Fee Schools | Share of schools where fees are waived — shows delta vs national average when filters are active |
| Critical LER | Schools where LER > 40 (resource-strained) |

### Critical Schools Drilldown
When any LER > 40 schools exist in the current selection, a collapsible table appears immediately below the KPI banner listing every critical school sorted by LER (worst first), with province, district, quintile, and urban/rural classification.
...
- ModeBar autohides on hover — chart titles are never obscured
...
| LER by Quintile Bar | Resource gap — LER rates from Q1 (poorest) through Q5 |
...
All analysis charts use a consistent autohide modeBar. Per-chart empty-state messages are shown when a filter combination produces no data for a given chart.
...
- Province (cascades to District options; label shows available district count)
- District
...
**Active filters** are shown as pill badges directly beneath the dashboard title so the current view scope is always visible.

**Reset all filters** — one-click button at the top of the sidebar clears every filter simultaneously.

**Export filtered data** — download the current filtered dataset as a CSV directly from the sidebar.

### Story Mode
Toggle **📖 Story** in the sidebar to inject curated narrative blocks above each visualisation, guiding users through spatial inequality, resource strain, and poverty in the SA education system.

### Dark / Light Mode
Sidebar toggle (**☀️ Light**) switches the full interface — shell, charts, and map — between a precision dark theme and a high-contrast light theme.
...
### v0.2.0 — 2026-05-14 · UX improvements

- **Active filter tags** — pill badges render beneath the dashboard title showing every active filter at a glance
- **Reset all filters** — one-click button at sidebar top clears all six filter widgets simultaneously
- **District count label** — District multiselect label shows the number of available districts for the current province selection
- **Story Mode / Light toggle moved to top** — both display toggles are now the first controls in the sidebar for discoverability
- **CSV export** — `⬇ Export filtered data` download button added to sidebar; produces the current filtered DataFrame as `emis_filtered.csv`
- **KPI deltas** — LER and No-Fee % cards show `▲/▼ vs national` comparison when any filter is active (national baseline loaded from cache, no extra DB call)
- **Critical schools drilldown** — collapsible expander below KPI banner lists all LER > 40 schools sorted by ratio, with province, district, quintile, and urban/rural
- **ModeBar autohide** — `displayModeBar: "hover"` applied to all charts; toolbar only appears on hover so chart titles are never obscured
- **Consistent chart config** — unified `_CHART_CFG` / `_MAP_CFG` constants replace per-call inline dicts
- **Per-chart empty states** — quintile bar, donut, and LER scatter each show a caption when the current filter yields no data for that chart
- **Contrast fix** — dark-theme `--text-3` token bumped from `#3d4352` to `#6b7280` (WCAG AA-compliant on dark backgrounds)
- **Light mode button legibility** — Reset and Export sidebar buttons have explicit light-mode colour overrides
